...
import protocols from 'dicom3d-web-project/src/protocols';
...
export default {
  ...
  actions: {
    ...
    WS_SHOW_BONE_CT({ state }) {
      if (state.client) {
        state.client.getRemote().Cone.showBoneCT().catch(console.error);
      }
    },
    WS_SHOW_ANGIO_CT({ state }) {
      if (state.client) {
        state.client.getRemote().Cone.showAngioCT().catch(console.error);
      }
    },
    WS_SHOW_MUSCLE_CT({ state }) {
      if (state.client) {
        state.client.getRemote().Cone.showMuscleCT().catch(console.error);
      }
    },
    WS_SHOW_MIP({ state }) {
      if (state.client) {
        state.client.getRemote().Cone.showMip().catch(console.error);
      }
    },
    WS_LIGHT({ state }) {
      if (state.client) {
        state.client.getRemote().Cone.light().catch(console.error);
      }
    },
    WS_CROP3D({ state }) {
      if (state.client) {
    ...
    WS_MEASURE_LENGTH({state}) {
      if (state.client) {
        state.client.getRemote().Cone.measureLength().catch(console.error);
      }
    },
  },
};
